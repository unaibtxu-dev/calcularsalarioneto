...
(function () {
  ...
  App.buildFormulario(form, {
    ...
    placeholder: "30.000",
    defaultValue: "30.000",
    territoriosSoportadosExtra: ["alava"],
    territorioDefecto: "alava"
  });

  function render(r, datos) {
    result.hidden = false;
    var resumen = App.resumenCondicionesHTML(datos, ["alava"]);
    if (r.bloqueado) {
    ...
    if (faqDinamica) {
      faqDinamica.textContent =
        (datos.territorio === "alava" ? "En Álava, " : "Con el territorio seleccionado, ") +
  ...
  function calcular() {
    ...
    // Álava usa el motor foral (el mismo que Bizkaia/Gipuzkoa, tabla
    // idéntica); el resto de territorios siguen con el común, para que el
    // selector siga siendo útil sin duplicar reglas fiscales.
