...
(function () {
  ...
  App.buildFormulario(form, {
    ...
    placeholder: "30.000",
    defaultValue: "30.000",
    territoriosSoportadosExtra: ["bizkaia"],
    territorioDefecto: "bizkaia"
  });

  function render(r, datos) {
    result.hidden = false;
    var resumen = App.resumenCondicionesHTML(datos, ["bizkaia"]);
    if (r.bloqueado) {
    ...
    if (faqDinamica) {
      faqDinamica.textContent =
        (datos.territorio === "bizkaia" ? "En Bizkaia, " : "Con el territorio seleccionado, ") +
  ...
  function calcular() {
    ...
    // Bizkaia usa el motor foral; el resto de territorios siguen con el
    // común, para que el selector siga siendo útil sin duplicar reglas
    // fiscales.
