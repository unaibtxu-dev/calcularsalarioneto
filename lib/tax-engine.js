...
(function (root, factory) {
  ...
  var TERRITORIOS_FORALES = { alava: true, bizkaia: true, gipuzkoa: true, navarra: true };
  var NUM_PAGAS_MIN = 12;
  var NUM_PAGAS_MAX = 20;

  // Número de pagas: solo cambia cómo se reparte el bruto/neto anual entre
  // pagas (ver netoPorPaga más abajo), nunca el propio cálculo de SS o IRPF
  // (art. 23.1.A RD 2064/1995: la cotización siempre prorratea /12; la
  // retención se calcula sobre el total anual previsto, no por paga). Fuera
  // del rango 12-20 o no numérico, se normaliza a 12 (mismo valor por
  // defecto que ya usaba esta función).
  function normalizarNumPagas(valor) {
    var n = Math.trunc(Number(valor));
    if (!Number.isFinite(n) || n < NUM_PAGAS_MIN || n > NUM_PAGAS_MAX) return NUM_PAGAS_MIN;
    return n;
  }
  ...
  function normalizarInput(input) {
    ...
    return {
      ...
      numPagas: normalizarNumPagas(i.numPagas),
      ...
      territorio: i.territorio || "comun", // 'comun' | 'ceuta' | 'melilla' | 'alava' | 'bizkaia' | 'gipuzkoa' | 'navarra'
